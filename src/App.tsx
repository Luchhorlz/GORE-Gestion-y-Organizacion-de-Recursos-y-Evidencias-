...
type AIChatMessage = { id: string; role: 'user' | 'assistant'; content: string; userProvided: boolean; sources: AssistantCitation[]; status: string; feedback?: { rating: 'useful' | 'incorrect' | 'review'; comment: string; updatedAt: string } | null; job?: AIChatJob; createdAt: string }
...
type AIStatus = { enabled: boolean; provider: string; available: boolean; configured: boolean; version: string; activeProfile: string; activeModel: string; profiles: { id: string; model: string; installed: boolean }[]; embeddingModel: string; embeddingInstalled: boolean }
    ...
    <div className={`app-shell ${presentation ? 'presentation' : ''}`}>
      ...
      <main>
        ...
        <div className="content">
          ...
          {view === 'configuracion' && <><GroqConnectCard /><AISettingsCard /><SettingsView config={caseConfig} onSaved={setCaseConfig} onLogout={logout} /></>}
        </div>
      </main>
...
function AIMessageFeedback({ message, onSaved }: { message: AIChatMessage; onSaved: () => void }) {
  ...
  async function save(rating: 'useful' | 'incorrect' | 'review', note = '') {
    setSaving(true)
    try { await apiPost(`/api/ai/chat/messages/${message.id}/feedback`, { rating, comment: note.trim() }); setEditing(null); onSaved() }
    finally { setSaving(false) }
  }
  if (message.role !== 'assistant' || message.status !== 'completed') return null
  return <div className="ai-message-feedback"><span>REVISIÓN HUMANA</span><div className="ai-feedback-actions"><button className={message.feedback?.rating === 'useful' ? 'selected useful' : ''} disabled={saving} onClick={() => save('useful')}><Check /> Útil</button><button className={message.feedback?.rating === 'incorrect' ? 'selected incorrect' : ''} disabled={saving} onClick={() => { setEditing('incorrect'); setComment(message.feedback?.rating === 'incorrect' ? message.feedback.comment : '') }}><X /> Incorrecta</button><button className={message.feedback?.rating === 'review' ? 'selected review' : ''} disabled={saving} onClick={() => { setEditing('review'); setComment(message.feedback?.rating === 'review' ? message.feedback.comment : '') }}><Clock3 /> Revisar</button></div>{editing && <div className="ai-feedback-note"><textarea maxLength={1000} value={comment} onChange={event => setComment(event.target.value)} placeholder={editing === 'incorrect' ? 'Explicá brevemente qué dato o interpretación debería corregirse…' : 'Anotá qué aspecto queda pendiente de revisión…'} /><div><small>{comment.length}/1000</small><button disabled={saving} onClick={() => save(editing, comment)}>{saving ? 'Guardando…' : 'Guardar revisión'}</button></div></div>}{message.feedback && !editing && <small className="ai-feedback-saved">{labels[message.feedback.rating]}{message.feedback.comment ? ` · ${message.feedback.comment}` : ''}</small>}</div>
}
...
function AIChatView() {
  ...
  async function send(event: React.FormEvent) {
    event.preventDefault(); const value = message.trim() || (attachments.length ? 'Analizá los archivos adjuntos y explicá sus datos relevantes, limitaciones y preguntas pendientes.' : ''); if (!value) return
    setSending(true); setError('')
    try {
      const uploaded: Evidence[] = []
      for (const file of attachments) uploaded.push(await apiUpload<Evidence>('/api/evidence', file))
      const result = await apiPost<AIConversation>('/api/ai/chat/messages', { conversationId: active?.id ?? null, message: value, evidenceIds: uploaded.map(item => item.id) })
      setActive(result); setMessage(''); setAttachments([]); refreshList()
    }
    catch { setError('No se pudo subir algún adjunto o poner el mensaje en la cola local. Revisá el archivo, Ollama e intentá nuevamente.') }
  ...
  function addAttachments(files: FileList | null) {
    ...
    setAttachments(previous => {
      const next = [...previous]
      for (const file of Array.from(files)) if (!next.some(item => item.name === file.name && item.size === file.size && item.lastModified === file.lastModified)) next.push(file)
      if (next.length > 10) setError('Podés adjuntar hasta 10 archivos en un mismo mensaje.')
...
function AIHistoryView() {
  ...
  return <><section className="page-heading compact with-action"><div><span className="eyebrow accent">MEMORIA TRAZABLE DEL EXPEDIENTE</span><h1>Historial de análisis</h1><p>Consultá resultados ya guardados sin volver a ocupar Ollama ni modificar el expediente.</p></div><button className="primary-button" onClick={refresh}><Search size={16} /> Actualizar</button></section><div className="ai-history-filters"><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>Todos <b>{items.length}</b></button>{Object.entries(labels).map(([type, label]) => <button className={filter === type ? 'active' : ''} onClick={() => setFilter(type)} key={type}>{label} <b>{items.filter(item => item.type === type).length}</b></button>)}</div>{error && <div className="login-error">{error}</div>}<div className="ai-history-list">{visible.map(item => <article className="panel ai-history-card" key={item.id}><div className="ai-history-icon"><Archive /></div><div className="ai-history-content"><div className="ai-history-head"><div><span>{labels[item.type] ?? item.type}</span><strong>{format(new Date(item.generatedAt), "dd/MM/yyyy 'a las' HH:mm")}</strong></div><b>Revisión humana requerida</b></div><p>{item.preview || 'Resultado guardado sin vista previa.'}</p><div className="ai-history-meta"><span><Sparkles /> {item.model}</span><span><FileCheck2 /> {item.sourceCount} fuente{item.sourceCount === 1 ? '' : 's'}</span><span><ShieldCheck /> Guardado localmente</span></div>{item.sources.length > 0 && <div className="chat-sources">{item.sources.map(source => <AISourceReference source={source} key={`${item.id}-${source.sourceId}`} />)}</div>}</div></article>)}{!loading && visible.length === 0 && <article className="panel vault-empty"><Archive /><strong>No hay análisis en esta categoría</strong><p>Cuando generes un resultado con alguna herramienta IA aparecerá automáticamente aquí.</p></article>}{loading && <article className="panel empty-inline">Recuperando análisis guardados…</article>}</div></>
}

function AIOperationsView() {
  const [status, setStatus] = useState<AIOperationsStatus | null>(null)
  const [error, setError] = useState('')
  const refresh = () => apiGet<AIOperationsStatus>('/api/ai/operations/status').then(result => { setStatus(result); setError('') }).catch(() => setError('No pudimos consultar el estado de las herramientas IA.'))
  useEffect(() => { refresh(); const timer = window.setInterval(refresh, 5000); return () => window.clearInterval(timer) }, [])
  const active = (status?.processingJobs.pending ?? 0) + (status?.processingJobs.processing ?? 0) + (status?.chatJobs.pending ?? 0) + (status?.chatJobs.processing ?? 0)
  const failures = (status?.processingJobs.failed ?? 0) + (status?.chatJobs.failed ?? 0) + (status?.evidence.failed ?? 0)
  const duration = status?.averageChatSeconds ? status.averageChatSeconds >= 60 ? `${Math.round(status.averageChatSeconds / 60)} min` : `${Math.round(status.averageChatSeconds)} s` : 'Sin datos'
  const statusLabel: Record<string, string> = { pending: 'En espera', processing: 'Trabajando', completed: 'Completado', failed: 'Falló', cancelled: 'Cancelado' }
  return <><section className="page-heading compact with-action"><div><span className="eyebrow accent">CONTROL LOCAL Y PRIVADO</span><h1>Estado de la IA</h1><p>Comprobá qué está haciendo Ollama, cuántas tareas quedan pendientes y si algún archivo necesita atención.</p></div><button className="primary-button" onClick={refresh}><Search size={16} /> Actualizar</button></section>{error && <div className="login-error">{error}</div>}<section className="summary-grid ai-operations-summary"><article className="summary-card"><div className={`summary-icon ${status?.ollamaAvailable ? 'green' : 'amber'}`}><Sparkles /></div><div><span>Ollama</span><strong>{status?.ollamaAvailable ? 'Conectado' : 'Sin conexión'}</strong><small>{status?.activeModel ?? 'Comprobando modelo…'}</small></div></article><article className="summary-card"><div className="summary-icon blue"><Clock3 /></div><div><span>Tareas activas</span><strong>{active}</strong><small>en espera o trabajando ahora</small></div></article><article className="summary-card"><div className={`summary-icon ${failures ? 'amber' : 'green'}`}><ShieldCheck /></div><div><span>Requieren atención</span><strong>{failures}</strong><small>errores acumulados del expediente</small></div></article></section><div className="ai-operations-grid"><article className="panel ai-operations-card"><div className="panel-head"><div><h2>Actividad acumulada</h2><p>Metadatos técnicos; no contiene mensajes ni documentos</p></div><ShieldCheck /></div><div className="ai-operation-metrics"><div><span>Respuestas de chat</span><strong>{status?.chatJobs.completed ?? 0}</strong></div><div><span>Análisis guardados</span><strong>{status?.completedAnalyses ?? 0}</strong></div><div><span>Procesamientos completados</span><strong>{status?.processingJobs.completed ?? 0}</strong></div><div><span>Tiempo medio del chat</span><strong>{duration}</strong></div><div><span>Evidencias del expediente</span><strong>{status?.evidence.total ?? 0}</strong></div><div><span>Extrayendo contenido</span><strong>{status?.evidence.extracting ?? 0}</strong></div></div></article><article className="panel ai-operations-card"><div className="panel-head"><div><h2>Últimas respuestas de Ollama</h2><p>El estado se actualiza automáticamente cada 5 segundos</p></div><Clock3 /></div><div className="ai-operations-list">{status?.latestChatJobs.map(job => <div key={job.id}><span className={`ai-operation-state ${job.status}`}>{statusLabel[job.status] ?? job.status}</span><div><strong>{job.stage}</strong><small>{job.model} · {format(new Date(job.updatedAt), 'dd/MM HH:mm')}</small></div><b>{job.progress}%</b></div>)}{status && status.latestChatJobs.length === 0 && <div className="empty-inline">Todavía no hay respuestas registradas.</div>}</div></article></div><div className="contradiction-warning"><Info /><div><strong>Los errores no eliminan ni modifican evidencias</strong><span>Los originales permanecen preservados. Un fallo indica que la extracción, indexación o respuesta puede necesitar un reintento.</span></div></div></>
}
...
function GroqConnectCard() {
  ...
  useEffect(() => { apiGet<AIStatus>('/api/ai/status').then(setStatus).catch(() => undefined) }, [])
