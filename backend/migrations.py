...
def _migration_007_chronology_proposals(db: sqlite3.Connection) -> None:
    db.executescript(
        """
        CREATE TABLE IF NOT EXISTS chronology_proposals (
            id TEXT PRIMARY KEY,
            tenant_id TEXT NOT NULL REFERENCES law_firms(id),
            case_id TEXT NOT NULL REFERENCES cases(id),
        ...
        CREATE INDEX IF NOT EXISTS idx_chronology_proposals_scope
            ON chronology_proposals (tenant_id,case_id,status,proposed_date);
        """
    )
...
def _migration_011_ai_conversation_archiving(db: sqlite3.Connection) -> None:
    _add_column(db, "ai_conversations", "archived_at TEXT")
    if _table_exists(db, "ai_conversations"):
        db.execute(
            "CREATE INDEX IF NOT EXISTS idx_ai_conversations_archive_scope "
            "ON ai_conversations (tenant_id,case_id,archived_at,updated_at)"
        )
